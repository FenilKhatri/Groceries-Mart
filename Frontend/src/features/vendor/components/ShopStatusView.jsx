...
const ShopStatusView = ({ shopData, onResubmit }) => {
  ...
  const status = (shopData?.status || SHOP_STATUS.PENDING).toLowerCase();
  const statusLabel = status.toUpperCase();

  const statusStyles = {
    [SHOP_STATUS.APPROVED]: "bg-emerald-100 text-emerald-700 border-emerald-200",
    [SHOP_STATUS.REJECTED]: "bg-red-100 text-red-700 border-red-200",
    [SHOP_STATUS.CANCELLED]: "bg-gray-100 text-gray-700 border-gray-200",
    [SHOP_STATUS.PENDING]: "bg-amber-50 text-amber-700 border-amber-200",
  };
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-xs font-semibold tracking-wide text-orange-600">
                {status === SHOP_STATUS.APPROVED ? "" : "SHOP REQUEST"}
              </p>
            ...
            <div
              className={`text-xs font-semibold px-3 py-2 rounded-2xl border ${
                statusStyles[status] || statusStyles[SHOP_STATUS.PENDING]
              }`}
            >
              {statusLabel}
            </div>
...
const DetailCard = ({ label, value, className = "" }) => (
  <div
    className={`rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3 ${className}`}
  >
